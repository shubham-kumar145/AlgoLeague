...
const ProblemPage = () => {
  ...
  const isAccepted = submitResult?.accepted === true;
  const isRejected = submitResult?.accepted === false;
  ...
  const statusBg = isAccepted
    ? 'bg-gradient-to-br from-[#32363b] to-[#2d3845]'
    : isRejected
      ? 'bg-gradient-to-br from-[#522828] to-[#500f0f]'
      : 'bg-gradient-to-br from-[#191E24] to-[#afc4df]';

  const statusTextColor = isAccepted ? "text-success" : "text-error";
...
">
              ...
              ${selectedLanguage === lang
                ...
                <select
                  name="editor_theme"
                  className="select select-sm bg-[#222a35] text-white"
                  onChange={(e) => set_editor_theme(e.target.value)}
                >
                  <option value="vs-dark">Dark</option>
                  <option value="vs-light">Light</option>
...
sm:static
      ...
      "
              >
                ...
                <div className="flex gap-2">
                  <button
                    className={`btn btn-outline text-white hover:bg-gray-700 btn-sm ${loading ? 'loading' : ''}`}
                    onClick={handleRun}
                    disabled={loading}
                  >
                    Run
      ...
      rounded-2xl shadow-xl p-4">
            ...
            ${runResult.success
                        ? 'bg-[#282c30]'
                        : 'bg-gradient-to-br from-[#351919] to-[#402525]'
                      }`}
                  >
                    {runResult.success ? (
                      <div className="space-y-6">
                        ...
                        <div className="space-y-3 max-h-[40vh] overflow-y-auto pr-2">
                          {runResult.testCases.map((tc, i) => (
                            <div
                              ...
                              className="bg-[#191E24] border border-[#15191E] p-4 rounded-xl text-xs"
                            >
                              <div className="font-mono space-y-1 text-white">
                                <div><strong>Input:</strong> {tc.stdin}</div>
                                <div><strong>Expected:</strong> {tc.expected_output}</div>
                                <div><strong>Output:</strong> {tc.stdout}</div>
                                <div
  ...
  rounded-2xl shadow-xl p-6">
              ...
              <div className="flex-1 overflow-y-auto pr-2 space-y-4">
                ...
                {submitResult ? (
                  <div
                    className={`rounded-2xl border border-[#15191E] p-5 shadow-sm ${statusBg}`}
                  >
                    <div className="space-y-6">
                      ...
                      <div className="flex flex-col gap-2 text-sm text-gray-300">
                        ...
                        <h4
                          className={`font-display font-bold ${statusTextColor} text-2xl tracking-tight`}
                        >
                          {statusTitle}
                        </h4>
